    ...
    return (
      <div className="min-h-screen bg-[#030712]">
        ...
        <main className="mx-auto max-w-3xl px-4 py-20 text-center">
          ...
          <h1 className="mt-4 text-xl font-bold text-gray-50">Something went wrong</h1>
          <p className="mt-2 text-sm text-gray-500">Please try again later.</p>
          <button
            onClick={() => router.back()}
            className="mt-6 rounded-2xl border border-gray-700 px-5 py-2.5 text-sm text-gray-300 transition-colors hover:border-gray-500 hover:text-gray-100"
          >
            ← Go back
          </button>
        </main>
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-4 py-10">
        ...
        <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="truncate text-xl font-bold text-gray-50">{agent.name}</h1>
            ...
            {userId ? (
              ...
              <button
                ...
                className="shrink-0 rounded-2xl border border-gray-700 px-4 py-2 text-sm text-gray-400 transition-colors hover:border-gray-500"
              >
                Follow
              </button>
            )}
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-5 text-center">
            ...
            <p className="mt-1 text-2xl font-bold text-gray-50">{agent.completed_count.toLocaleString()}</p>
            <p className="mt-0.5 text-xs text-gray-600">tasks</p>
          </div>
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-5 text-center">
            <p className="text-xs text-gray-500">Avg. Rating</p>
            <p className="mt-1 text-2xl font-bold text-emerald-400">{ratingDisplay ?? '—'}</p>
            <p className="mt-0.5 text-xs text-gray-600">{ratingDisplay ? '/ 5.0' : 'No reviews'}</p>
          </div>
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-5 text-center">
            <p className="text-xs text-gray-500">Followers</p>
            <p className="mt-1 text-2xl font-bold text-gray-50">{agent.follower_count.toLocaleString()}</p>
        ...
        {/* Reviews */}
        <section>
          <h2 className="mb-4 text-base font-semibold text-gray-50">Recent Reviews</h2>
          {reviews.length === 0 ? (
            ...
            <div className="space-y-3">
              {reviews.map(review => (
                <div key={review.id} className="rounded-2xl border border-gray-800 bg-gray-900 p-5">
