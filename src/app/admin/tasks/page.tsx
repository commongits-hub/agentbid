  ...
  return (
    <div className="px-8 py-8">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-50">Tasks</h1>
          <p className="mt-0.5 text-sm text-gray-500">Recent 50 active tasks</p>
        </div>
        {actionMsg && (
          ...
          <div className="mb-6 flex flex-wrap gap-3">
            {disputed > 0 && (
              <div className="flex items-center gap-2 rounded-xl border border-red-800/60 bg-red-950/20 px-4 py-2.5">
                ...
                <span className="text-sm font-semibold text-red-400">{disputed}</span>
            ...
            {reviewing > 0 && (
              <div className="flex items-center gap-2 rounded-xl border border-amber-800/60 bg-amber-950/20 px-4 py-2.5">
                ...
                <span className="text-sm font-semibold text-amber-400">{reviewing}</span>
                <span className="text-xs text-amber-600">Reviewing</span>
              </div>
            )}
            <div className="flex items-center gap-2 rounded-xl border border-gray-800 bg-gray-900 px-4 py-2.5">
              <span className="text-sm font-semibold text-gray-300">{open}</span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900/60">
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500">Submissions</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {tasks.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-600">No tasks found.</td></tr>
              ) : (
                tasks.map(t => (
                  <tr key={t.id} className={`hover:bg-gray-900/40 transition-colors ${t.status === 'disputed' ? 'bg-red-950/10' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-gray-500">{new Date(t.created_at).toLocaleDateString('en-US')}</td>
      ...
      <ConfirmDialog
        ...
        description={confirm ? `Change "${confirm.title.slice(0, 30)}" to "${STATUS_LABELS[confirm.nextStatus]}"?` : ''}
        confirmLabel="Change"
